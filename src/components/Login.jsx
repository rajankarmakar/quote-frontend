...
function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        if ( '' === currentState.email || '' === currentState.password ) {
            currentState.error = 'Email or Password can not be empty';
            setState({ currentState });
            return;
        }
        
        try {
            const {data: token} = await login( currentState.email, currentState.password );
            localStorage.setItem('token', token);
            window.location = "/";
        } catch (ex) {
            if ( ex.response && 400 === ex.response.status ) {
                currentState.error = 'Email or Password can not be empty';
                setState({ currentState });
            }
        }
